...
public:
	inline explicit tablet(void) noexcept {
		...
		for (unsigned long index = 0;; ++index) {
			...
			_handle = CreateFileW(detail_data->DevicePath, FILE_READ_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_DEVICE /* | FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH*/, nullptr);
			free(detail_data);
